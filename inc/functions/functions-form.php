...
function getCurrentCountry() {
    ...
    if ( is_array( $events_online )):

        ob_start();

        $needed_event_location_prague = [];
        $needed_event_location_world = [];

        foreach ( $events_online as $item )
        {

            $packages_online = get_field('packages', $item->ID);

            foreach ($packages_online as $single_location)
            {
                if ($single_location['location'] === 'prague') {
                    array_push($needed_event_location_prague, $item->ID);
                    $needed_event_location_prague = array_unique($needed_event_location_prague);
                }
                array_push($needed_event_location_world, $item->ID);
                $needed_event_location_world = array_unique($needed_event_location_world);
            }
        ...
        {
            $correct_arr = array_unique($needed_event_location_prague);
        }
        else{
            $correct_arr = array_unique($needed_event_location_world);
        }


        if ( $events_online ) {
            $result = array_unique($correct_arr);
...
function getCurrentEvent() {
    ...
    if($type_event == "Online"){
        ...
        $event_package = get_field('packages_offline', $event_package_id);
    }
    ...
    if ( is_array($event_package) ) :
        ...
        <script>
             delete dates_array;
             dates_array = <?php echo json_encode( get_post_meta( $event_package_id, 'datepicker_value', true ) ); ?>;
        </script>
        <?php
        if ( $current_country != 'Prague') {

            foreach ($event_package as $item):
                $svg = get_field('svg_name', $event_package_id);
                ?>
                <div class="package">
                    <h3 class="title"><?php echo $item['name'] ?></h3>
                    <div class="svg">
                        <?php
                        $svg_name = get_field( 'svg_name', $event_package_id);
